...
async function testAPIWithAuth() {
  ...
  for (const endpoint of endpoints) {
    ...
    if (response.ok) {
      ...
      console.log(
        `   Data type: ${Array.isArray(data.data) ? "array" : typeof data.data}`
      );
      ...
      console.log(
        `⚠️  ${endpoint.name} API response: ${response.status} ${response.statusText}`
      );
  ...
  console.log(
    `✅ No auth test: ${noAuthResponse.status} ${noAuthResponse.statusText}`
  );
