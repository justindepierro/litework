...
export default function ProgressionTracking({ 
  ...
  const generateProgressionSuggestions = (): ProgressionSuggestion[] => {
    ...
    const currentReps = typeof exercise.reps === 'string' 
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-900">Progression Tracking</h3>
          <p className="text-sm text-gray-600">Smart suggestions for {exercise.exerciseName}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-blue-600">{exercise.weight || 0}</div>
        ...
        <div className="bg-purple-50 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-purple-600">{exercise.sets}</div>
          ...
          <div className="space-y-2">
            {historicalData.slice(-3).map((data, index) => (
              <div key={index} className="flex items-center justify-between text-sm bg-gray-50 rounded-lg p-2">
                ...
                <span className="font-medium">{data.weight}lbs × {data.reps} × {data.sets}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        {progressionSuggestions.length > 0 ? (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold capitalize">{suggestion.type} Progression</span>
          ...
          <div className="text-center py-6 text-gray-500">
            ...
            <p className="text-xs">Complete more workouts to unlock smart progressions</p>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => onUpdate({ ...exercise, weight: parseInt(e.target.value) })}
          ...
          <div>
            ...
            <input
              type="number"
              value={typeof exercise.reps === 'number' ? exercise.reps : parseInt(exercise.reps)}
