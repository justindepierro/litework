...
export type MetadataProperties = Record<string, string | number | boolean | Date | null | undefined>;
...
class AnalyticsService {
  ...
  trackWorkoutStarted(workoutId: string, athleteId: string, metadata?: MetadataProperties) {
    this.track({
  ...
  trackFeatureUsage(feature: string, action: string, metadata?: MetadataProperties) {
    this.track({
      name: 'feature_used',
      properties: {
  ...
  // A/B test tracking
  trackExperiment(experiment: string, variant: string, metadata?: MetadataProperties) {
    this.track({
...
declare global {
  interface Window {
    va?: (event: string, name: string, properties?: AnalyticsProperties) => void;
