...
function DashboardClientPageComponent({
  ...
  }, [user?.id, isCoachOrAdmin, hasInitialCoachPayload]);
  ...
  const debugInfo = `Loading: ${isLoading}, User: ${user ? user.email : "null"}, Role: ${user?.role || "none"}, IsCoachOrAdmin: ${isCoachOrAdmin}`;
      ...
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-sky-50 to-purple-50">
        <div className="text-center">
          ...
          {process.env.NODE_ENV === "development" && (
            <Caption variant="muted" className="mt-4 font-mono">
              {debugInfo}
            </Caption>
          )}
        </div>
  ...
  // If no user after loading completes, show auth required and let guard redirect
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-sky-50 to-purple-50 p-4">
        <Card variant="elevated" padding="lg" className="max-w-md text-center">
          ...
          {process.env.NODE_ENV === "development" && (
            <Caption
              variant="muted"
              className="mb-4 font-mono text-left bg-silver-100 p-2 rounded"
            >
              Debug: {debugInfo}
            </Caption>
          )}
  ...
  const dashboardJSX = (
    <>
      <div
        className="min-h-screen bg-linear-to-br from-sky-50 via-blue-50 to-indigo-50 pt-16 sm:pt-18"
        style={{
          position: "relative",
          zIndex: 1,
          minHeight: "100vh",
        ...
        <div className="container-responsive px-4 py-5 pb-24 max-w-2xl mx-auto space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href="/schedule">
                  <Button
                    ...
                    className="glass hover:glass-thick transition-all"
                  >
                    Schedule
                  </Button>
                </Link>
          ...
          <section>
            <Suspense fallback={<SkeletonCard />}>
              <QuickStatsWidget athleteId={user?.id} />
            </Suspense>
          </section>
          ...
          <section>
            <Suspense fallback={<SkeletonCard />}>
              <RecentWorkouts athleteId={user?.id} limit={3} />
            </Suspense>
          </section>
