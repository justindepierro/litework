...
const enhancedAthletes: EnhancedAthlete[] = [
  {
    ...
    personalRecords: [
      { 
        id: "kpi1", athleteId: "1", exerciseId: "bench", exerciseName: "Bench Press", 
        currentPR: 285, dateAchieved: new Date("2024-10-15"), isActive: true,
        createdAt: new Date("2024-01-15"), updatedAt: new Date("2024-10-15")
      },
      { 
        id: "kpi2", athleteId: "1", exerciseId: "squat", exerciseName: "Back Squat", 
        currentPR: 385, dateAchieved: new Date("2024-10-20"), isActive: true,
        createdAt: new Date("2024-01-15"), updatedAt: new Date("2024-10-20")
      }
    ],
  ...
  },
  {
    ...
    communication: {
      unreadMessages: 0,
      lastMessage: "Thanks for adjusting my workout for the ankle. Feeling better!",
    ...
    personalRecords: [
      { 
        id: "kpi3", athleteId: "2", exerciseId: "vertical", exerciseName: "Vertical Jump", 
        currentPR: 28, dateAchieved: new Date("2024-09-15"), isActive: true,
        createdAt: new Date("2024-01-20"), updatedAt: new Date("2024-09-15")
      }
    ],
...
export default function AthletesPage() {
  ...
  const handleAddKPI = async () => {
    ...
    setKPIForm({ kpiName: "", value: "", dateSet: new Date().toISOString().split("T")[0] });
  };
  ...
  const getStatusIcon = (status: string, injuryStatus?: string | null) => {
    if (injuryStatus) return <AlertCircle className="h-4 w-4 text-yellow-500" />;
    if (status === "active") return <CheckCircle className="h-4 w-4 text-green-500" />;
    if (status === "invited") return <Clock className="h-4 w-4 text-blue-500" />;
        ...
        <div className="flex flex-col gap-6 mb-8">
          <div className="text-center sm:text-left">
            ...
            <p className="text-heading-secondary text-sm sm:text-base lg:text-lg max-w-2xl leading-relaxed mx-auto sm:mx-0">
              Manage your athletes with invite-based onboarding, performance tracking, and seamless communication
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 sm:gap-6">
            <div className="flex items-center justify-center sm:justify-start gap-2 p-3 bg-blue-50 rounded-xl sm:bg-transparent sm:p-0">
              ...
              <span className="text-sm font-medium">{athletes.filter(a => a.status === "active").length} Active</span>
            </div>
            <div className="flex items-center justify-center sm:justify-start gap-2 p-3 bg-orange-50 rounded-xl sm:bg-transparent sm:p-0">
              ...
              <span className="text-sm font-medium">{athletes.filter(a => a.status === "invited").length} Pending</span>
            </div>
            <div className="flex items-center justify-center sm:justify-start gap-2 p-3 bg-green-50 rounded-xl sm:bg-transparent sm:p-0">
              ...
              <span className="text-sm font-medium">{athletes.reduce((acc, a) => acc + (a.communication?.unreadMessages || 0), 0)} Unread</span>
            </div>
            <div className="flex items-center justify-center sm:justify-start gap-2 p-3 bg-yellow-50 rounded-xl sm:bg-transparent sm:p-0">
              ...
              <span className="text-sm font-medium">{athletes.filter(a => a.injuryStatus).length} Injured</span>
            </div>
          </div>
        ...
        {filteredAthletes.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
            {filteredAthletes.map((athlete) => (
              <div key={athlete.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-200 group touch-manipulation">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="relative shrink-0">
                        ...
                        {athlete.communication?.unreadMessages && athlete.communication.unreadMessages > 0 && (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-600">{getStatusText(athlete.status, athlete.injuryStatus)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-blue-900 line-clamp-2">{athlete.communication.lastMessage}</p>
                          <p className="text-xs text-blue-600 mt-1">
                            {athlete.communication.lastMessageTime?.toLocaleDateString()} at {athlete.communication.lastMessageTime?.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 p-3 bg-gray-50 rounded-lg">
                      <div className="text-center">
                        <div className="flex items-center justify-center gap-1 mb-1">
                          ...
                          <span className="font-semibold text-gray-900">{athlete.stats.thisMonthWorkouts}</span>
                        </div>
                      ...
                      <div className="text-center">
                        <div className="flex items-center justify-center gap-1 mb-1">
                          ...
                          <span className="font-semibold text-gray-900">{athlete.stats.recentPRs}</span>
                        </div>
                  ...
                  {athlete.personalRecords && athlete.personalRecords.length > 0 && (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-gray-700">Top PRs</span>
                      </div>
                      <div className="space-y-1">
                        {athlete.personalRecords.slice(0, 2).map((pr: AthleteKPI) => (
                          <div key={pr.id} className="flex justify-between items-center text-sm">
                            <span className="text-gray-600">{pr.exerciseName}:</span>
                            <span className="font-medium text-gray-900">{pr.currentPR} lbs</span>
                          </div>
                        ))}
                        {athlete.personalRecords.length > 2 && (
                          <p className="text-xs text-gray-500">+{athlete.personalRecords.length - 2} more</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {athlete.status === "invited" && (
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-blue-900">Invite Pending</span>
                      </div>
                      <p className="text-xs text-blue-700">Sent {athlete.createdAt.toLocaleDateString()}</p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-900">Invite New Athlete</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-blue-900 mb-1">How Invites Work</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setMessageForm({...messageForm, priority: e.target.value as "low" | "normal" | "high"})}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    >
                  ...
                  <div className="flex items-end">
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setMessageForm({...messageForm, notifyViaEmail: e.target.checked})}
                        className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                      />
                      <span className="text-sm text-gray-700">Also send via email</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-green-900 mb-1">Communication Tip</h4>
                      <p className="text-sm text-green-700">
                        {selectedAthlete.communication?.preferredContact === "email" 
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h3 className="text-lg font-medium text-gray-900 mb-3">Add New Personal Record</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setKPIForm({...kpiForm, dateSet: e.target.value})}
                      className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="flex items-end">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-3">
                  Current Personal Records ({selectedAthlete.personalRecords?.length || 0})
                </h3>
                {selectedAthlete.personalRecords?.length ? (
                  <div className="space-y-3">
                    {selectedAthlete.personalRecords.map((kpi: AthleteKPI) => (
                      <div key={kpi.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                        <div className="flex items-center gap-3">
                          <Target className="w-5 h-5 text-blue-600" />
                          <div>
                            <div className="font-medium text-gray-900">{kpi.exerciseName}</div>
                            <div className="text-sm text-gray-600">
                              {kpi.currentPR} lbs • {kpi.dateAchieved.toLocaleDateString()}
                            </div>
                          </div>
                        </div>
