  ...
  try {
    ...
    switch (type) {
      ...
      case 'bulk_update_status':
        result = await handleBulkStatusUpdate(targetAthletes, targetGroups, data);
        break;
      case 'bulk_assign_workout':
        result = await handleBulkWorkoutAssignment(targetAthletes, targetGroups, data);
        break;
      default:
        return NextResponse.json({ 
...
async function handleBulkInvite(targetAthletes: string[], targetGroups: string[], data: {
  groupIds?: string[];
  message?: string;
...
async function handleBulkMessage(targetAthletes: string[], targetGroups: string[], data: {
  subject: string;
  message: string;
...
async function handleBulkStatusUpdate(targetAthletes: string[], targetGroups: string[], data: {
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to update athlete status'
    };
  }
}

async function handleBulkWorkoutAssignment(targetAthletes: string[], targetGroups: string[], data: {
  workoutId: string;
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to assign workouts'
    };
  }
}
