...
async function runMigration() {
  ...
  try {
    // Read the SQL file
    const sqlPath = join(__dirname, '../../database/add-user-creation-trigger.sql');
    ...
    console.log('✨ Migration complete! New users will automatically get profiles.');

  } catch (error) {
    console.error('❌ Migration failed:', error);
    console.error('\n📝 Manual Steps:');
    console.error('   1. Go to Supabase Dashboard > SQL Editor');
    console.error('   2. Run the SQL from: database/add-user-creation-trigger.sql');
