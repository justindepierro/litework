...
async function checkRecentAthletes() {
  ...
  try {
    ...
    console.log(`📊 ATHLETES CREATED IN LAST 30 DAYS: ${athletes?.length || 0}\n`);
    ...
    if (activeAthletes.length > 0) {
      ...
      activeAthletes.forEach((athlete, index) => {
        const daysSinceCreated = Math.floor((new Date() - new Date(athlete.created_at)) / (1000 * 60 * 60 * 24));
        
        console.log(`${index + 1}. ${athlete.first_name} ${athlete.last_name}`);
        console.log(`   Email: ${athlete.email}`);
        console.log(`   Created: ${new Date(athlete.created_at).toLocaleDateString()} (${daysSinceCreated} days ago)`);
    ...
    if (deletedAthletes.length > 0) {
      ...
      deletedAthletes.forEach((athlete, index) => {
        const daysSinceCreated = Math.floor((new Date() - new Date(athlete.created_at)) / (1000 * 60 * 60 * 24));
        const daysSinceDeleted = Math.floor((new Date() - new Date(athlete.deleted_at)) / (1000 * 60 * 60 * 24));
        
        console.log(`${index + 1}. ${athlete.first_name} ${athlete.last_name}`);
        console.log(`   Email: ${athlete.email}`);
        console.log(`   Created: ${new Date(athlete.created_at).toLocaleDateString()} (${daysSinceCreated} days ago)`);
        console.log(`   Deleted: ${new Date(athlete.deleted_at).toLocaleDateString()} (${daysSinceDeleted} days ago)`);
        console.log(`   ID: ${athlete.id}`);
    ...
    if (auditError) {
      ...
      auditLogs.forEach((log, index) => {
        console.log(`${index + 1}. Deleted at: ${new Date(log.performed_at).toLocaleDateString()}`);
