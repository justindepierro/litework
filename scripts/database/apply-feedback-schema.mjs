...
async function applySchemaMigration() {
  ...
  try {
    // Read the SQL file
    const sqlPath = join(__dirname, '../../database/workout-session-feedback-schema.sql');
    ...
    if (!tableError) {
      ...
      console.log('⚠️  Note: Could not verify table (this is normal if no data exists yet)\n');
    }
