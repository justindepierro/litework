...
async function checkPendingInvites() {
  ...
  try {
    ...
    const { data: allInvites, error: allError } = await supabase
      .from('invites')
      .select('id, email, first_name, last_name, status, expires_at, created_at, deleted_at')
      .in('status', ['pending', 'draft'])
      .order('created_at', { ascending: false });

    if (allError) throw allError;

    console.log(`📊 TOTAL INVITES (pending/draft): ${allInvites?.length || 0}\n`);
    ...
    const expiredInvites = allInvites.filter(i => i.expires_at && new Date(i.expires_at) < new Date() && !i.deleted_at);
