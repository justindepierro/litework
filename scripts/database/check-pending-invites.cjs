...
async function checkPendingInvites() {
  ...
  try {
    ...
    const { data: allInvites, error: allError } = await supabase
      .from("invites")
      .select(
        "id, email, first_name, last_name, status, expires_at, created_at, deleted_at, accepted_at"
      )
    ...
    // Separate by status
    const pendingActive = allInvites.filter(
      (i) => (i.status === "pending" || i.status === "draft") && !i.deleted_at
    );
    ...
    const cancelledInvites = allInvites.filter(
      (i) => i.status === "cancelled" && !i.deleted_at
    );
    ...
    if (acceptedInvites.length > 0) {
      ...
      acceptedInvites.forEach((invite, index) => {
        const daysSinceAccepted = invite.accepted_at
          ? Math.floor(
              (new Date() - new Date(invite.accepted_at)) /
                (1000 * 60 * 60 * 24)
            )
          : null;
        ...
        if (invite.accepted_at) {
          console.log(
            `   Accepted: ${new Date(invite.accepted_at).toLocaleDateString()} (${daysSinceAccepted} days ago)`
          );
        }
        console.log(`   ID: ${invite.id}\n`);
      });
