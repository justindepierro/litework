...
console.log(
  `Found ${filesToProcess.length} files with console.log statements\n`
);
...
filesToProcess.forEach((file, index) => {
  console.log(
    `  ${index + 1}. ${file.relativePath} (${file.lines.length} occurrences)`
  );
});
...
rl.question(
  "Do you want to comment out all console.log statements? (yes/no): ",
  (answer) => {
    rl.close();
