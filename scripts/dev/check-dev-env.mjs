...
function checkPortAvailability() {
  ...
  try {
    ...
    if (result) {
      ...
      try {
        const processInfo = execSync(`ps aux | grep ${result} | grep -v grep`, {
          encoding: "utf8",
        }).trim();
...
function checkProjectStructure() {
  ...
  const requiredDirs = ["src", "src/app", "src/components", "src/types"];
...
function checkNextCache() {
  ...
  if (existsSync(nextDir)) {
    try {
      ...
      if (ageHours > 24) {
        log(
          `⚠️  .next directory is ${Math.round(ageHours)} hours old`,
          "yellow"
        );
...
function generateRecommendations(issues) {
  if (issues.length === 0) {
    log(
      "\n🎉 All checks passed! Your development environment looks good.",
      "green"
    );
    return;
  }
  ...
  const recommendations = {
    nodeVersion:
      "• Upgrade to Node.js 18+ for better performance and compatibility",
    memory:
