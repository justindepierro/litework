...
const PATTERNS = {
  ...
  dateColumnWithISOString: /\.update\s*\(\s*\{[^}]*(?:scheduled_date|assigned_date|due_date|date):\s*[^}]*\.toISOString\(\)/gs,
  
  // TIMESTAMP column getting date-only format - potentially wrong
  timestampColumnWithDateOnly: /\.update\s*\(\s*\{[^}]*(?:created_at|updated_at|started_at|completed_at|read_at):\s*[^}]*\.split\(['"]T['"]\)\[0\]/gs,
...
function scanFile(filePath) {
  try {
    ...
    let match;
    const dateColumnMatches = [...content.matchAll(PATTERNS.dateColumnWithISOString)];
    ...
    for (const match of dateColumnMatches) {
      ...
      issues.push({
        ...
        file: relativePath,
        line: lineNumber,
        message: 'DATE column receiving toISOString() - should use .split("T")[0] for date-only',
        snippet: snippet,
    ...
    for (const match of fetchMatches) {
      ...
      const hasDateConversion = afterFetch.includes('new Date(') || afterFetch.includes('toISOString().split');
      ...
      if (!hasDateConversion && afterFetch.includes('setAssignments')) {
        ...
        issues.push({
          ...
          file: relativePath,
          line: lineNumber,
          message: 'Fetched data may contain date strings not converted to Date objects',
          snippet: match[0],
      ...
      for (const [table, columns] of Object.entries(DATABASE_SCHEMA)) {
        for (const [column, type] of Object.entries(columns)) {
          if (type === 'DATE' && updateBlock.includes(column)) {
            if (updateBlock.includes('.toISOString()') && !updateBlock.includes('.split')) {
              const lineNumber = content.substring(0, match.index).split('\n').length;
              issues.push({
                severity: 'HIGH',
...
function generateReport() {
  ...
  console.log(`  HIGH:   ${bySeverity.HIGH.length} (Critical bugs - fix immediately)`);
  console.log(`  MEDIUM: ${bySeverity.MEDIUM.length} (Potential bugs - review)`);
  ...
  Object.entries(byFile)
    ...
    .forEach(([file, fileIssues]) => {
      ...
      console.log(`\n${file}`);
      console.log(`  Total: ${fileIssues.length} issues (HIGH: ${highCount}, MEDIUM: ${mediumCount})`);
