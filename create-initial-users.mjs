...
async function createInitialUsers() {
  ...
  for (const userData of users) {
    try {
      ...
      if (existingUser) {
        console.log(`📧 Auth user already exists: ${userData.email} (${existingUser.id})`);
        ...
        console.log(`✅ Created auth user: ${userData.email} (${authData.user.id})`);
      ...
      if (existingProfile) {
        ...
        if (profileError) {
          console.error(`❌ Failed to create user profile ${userData.email}:`, profileError.message);
          continue;
        }
